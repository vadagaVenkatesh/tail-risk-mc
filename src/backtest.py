...
def main():
    ...
    with open(C.RESULTS) as f:
        results = json.load(f)
    # Conditional model: a different VaR every day (see conditional.py).
    ...
    for p in C.ALPHAS:
        ...
        for model in ("normal", "student_t", "gpd", "garch_evt"):
            if model == "garch_evt":
                var = cond[f"VaR_{p}"].to_numpy()
            else:
            ...
            results["backtest"][f"{p}"][model] = {
                "VaR": float(np.mean(var)), "breaches": breaches, "rate": rate,
            ...
            print(f"   {model:10s} VaR={np.mean(var):.4f}  breaches={breaches:3d} "
